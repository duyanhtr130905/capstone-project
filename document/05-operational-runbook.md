...
**Tình trạng hiện tại:** Quyền `owner` được gán cho một ví đơn (EOA — Externally Owned Account), không phải multisig hay timelock contract. Đây là rủi ro đã ghi nhận tại mục 6 của `01-threat-model.md`. Toàn bộ nội dung từ mục 3 trở đi giả định vai trò `owner` do một cá nhân/nhóm nhỏ nắm giữ private key trực tiếp.
...
- **Lưu ý quan trọng:** `pause()` **không chặn** `claimReward()` (theo thiết kế có chủ đích — xem `01-threat-model.md`, T-03). Vì vậy, `pause()` không phải công cụ để "khóa" toàn bộ tài sản người dùng — chỉ ngăn giao dịch stake/unstake mới.
...
| 5. Khắc phục | Theo `04-deployment-runbook.md` mục 5 (không có rollback — có thể cần deploy contract mới) | Người vận hành / kỹ thuật |
...
Liên quan tới rủi ro kinh tế E-01 đã ghi nhận trong `03-tokenomics-note.md` (thưởng thất thoát nếu không ai stake), trước khi gọi `notifyRewardAmount` cho chu kỳ mới:
...
3. Thực hiện theo đúng thứ tự: chuyển token trước, `notifyRewardAmount` sau (xem `03-tokenomics-note.md` mục 5).
4. Xác nhận `rewardRate > 0` sau khi gọi — nếu bằng 0 (do làm tròn, xem `01-threat-model.md` T-04), lượng `reward` cấp lần đó không đủ so với `rewardsDuration`, cần cấp lại với số lượng lớn hơn.

## 8. Quản lý khóa (Key Management)

- Private key của ví `owner` không được lưu dưới dạng plaintext trong bất kỳ file nào commit vào Git (đã tuân thủ — xem `.env.example` trong `04-deployment-runbook.md`).
- Khuyến nghị sao lưu (backup) seed phrase ở nơi an toàn, tách biệt khỏi thiết bị dùng để vận hành hàng ngày.
- Khuyến nghị dài hạn: chuyển quyền `owner` sang ví multisig (yêu cầu nhiều chữ ký để thực thi 1 giao dịch quản trị) trước khi vận hành với tài sản giá trị thật — xem khuyến nghị P3 trong `01-threat-model.md`.
...
---
*Đây là tài liệu cuối cùng trong bộ 5 tài liệu kỹ thuật retroactive: `01-threat-model.md`, `02-onchain-offchain-data-map.md`, `03-tokenomics-note.md`, `04-deployment-runbook.md`, `05-operational-runbook.md`.*
